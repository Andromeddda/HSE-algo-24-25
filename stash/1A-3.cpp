...
const digit_t base = 1000;
const digit_t char_base = 10;
const size_t digit_length = 3;
const size_t naive_size = 2;

// Console utils
pair<Number, bool> read_number(istream& is);
pair<Number, bool> make_number(const string& str);
...
test_karatsuba() {
    while (true) {
        ...
        pair<Number, bool> x = make_number(to_string(a));
        pair<Number, bool> y = make_number(to_string(b));

        auto z = Karatsuba(x.first, y.first);
        auto znaive = naive_multiplication(x.first, y.first);

        if (z != znaive) {
            cout << "bad karatsuba: " << a << " " << b << '\n';
            break;
        }
    }
}
...
test_addition() {   
    while (true) {
        long long a = std::rand();
        long long b = std::rand();

        pair<Number, bool> x = make_number(to_string(a));
        pair<Number, bool> y = make_number(to_string(b));
        pair<Number, bool> z = make_number(to_string(a + b));

        if (add(x.first, y.first) != z.first) {
            cout << "bad addition: " << a << " " << b << '\n';
            break;
        }
        else {
            cout << a << "+" << b << '\n';
...
test_subtraction() {   
    while (true) {
        ...
        if (subtract(x.first, y.first) != z.first) {
            cout << "bad subtraction: " << A << " " << B << '\n';
            break;
        }
    }
}
